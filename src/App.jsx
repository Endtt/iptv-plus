...
export default function IPTVApp() {
  ...
  const loadFromUrl = async () => {
    ...
    // Sırayla farklı proxy'leri dene
    const proxies = [
      (u) => u,
    ...
    for (const makeUrl of proxies) {
      try {
        ...
        if (t.includes("#EXTINF") || t.includes("#EXTM3U")) { text = t; break; }
      } catch(e) { continue; }
    }

    if (!text) {
      setLoadErr("Listeye ulaşılamadı ❌  URL'yi kontrol edin ya da dosyayı indirip 'Dosyadan Yükle' ile deneyin.");
      setLoading(false);
      return;
    }
    ...
    if (!chs.length) {
      setLoadErr("Hiç kanal bulunamadı. M3U formatını kontrol edin.");
      setLoading(false);
      return;
    }
            ...
            {importTab==="url" ? (
              <div>
                ...
                <button onClick={loadFromUrl} disabled={loading} style={{
                  ...
                  {loading ? "⏳ Proxy'ler deneniyor..." : "✅ Listeyi Yükle"}
                </button>
                <div style={{ marginTop:"10px", padding:"10px", background:"rgba(255,255,255,0.03)", borderRadius:"10px" }}>
                  <div style={{ fontSize:"10px", color:"#555", lineHeight:"1.6" }}>
                    ...
                    <span style={{ color:"#444", fontFamily:"monospace" }}>http://sunucu:port/get.php?username=...&password=...&type=m3u</span><br/><br/>
                    ⚠️ URL çalışmazsa: M3U dosyasını bilgisayara indirip <strong style={{color:"#a78bfa"}}>Dosyadan Yükle</strong> seçeneğini deneyin.
                  </div>
                </div>
              </div>
            ) : (
